...
CILIUM_VER="v0.18.2"; curl -L https://github.com/cilium/cilium-cli/releases/download/${CILIUM_VER}/cilium-$(uname -s | tr '[:upper:]' '[:lower:]')-$(unmae -m).tar.gz | tar zxf -

DUCKDB_VER="v1.2.1"; curl -LO "https://github.com/duckdb/duckdb/releases/download/${DUCKDB_VER}/duckdb_cli-osx-universal.zip" | unzip -oqK - duckdb

HELM_VER="3.17.2"; curl -L "https://get.helm.sh/helm-v${HELM_VER}-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m).tar.gz" | tar zxf - --strip-components 1 "$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)/helm"
...
K9S_VER="v0.40.10"; curl -L "https://github.com/derailed/k9s/releases/download/${K9S_VER}/k9s_$(uname -s)_$(uname -m).tar.gz" | tar zxf - k9s
...
TFORM_VER="1.11.2"; curl -LOSfs "https://releases.hashicorp.com/terraform/${TFORM_VER}/terraform_${TFORM_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip" && unzip -oqK terraform_${TFORM_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip terraform
...
0.36.4/terraform-ls_0.36.4_darwin_arm64.zip

TRIVY_VER="0.60.0"; curl -L "https://github.com/aquasecurity/trivy/releases/download/v${TRIVY_VER}/trivy_${TRIVY_VER}_macOS-ARM64.tar.gz" | tar zxf - trivy
...
VAULT_VER="1.19.0"; curl -LO "https://releases.hashicorp.com/vault/${VAULT_VER}/vault_${VAULT_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip" && unzip -oqK vault_${VAULT_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip vault
