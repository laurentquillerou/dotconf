...
CILIUM_VER="v0.18.9"; curl -L "https://github.com/cilium/cilium-cli/releases/download/${CILIUM_VER}/cilium-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m).tar.gz" | tar zxf -

DUCKDB_VER="v1.4.3"; curl -LO "https://github.com/duckdb/duckdb/releases/download/${DUCKDB_VER}/duckdb_cli-osx-universal.zip" | unzip -oqK - duckdb

HELM_VER="4.0.4"; curl -L "https://get.helm.sh/helm-v${HELM_VER}-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m).tar.gz" | tar zxf - --strip-components 1 "$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m)/helm"
...
JJ_VER="v0.36.0"; curl -L "https://github.com/jj-vcs/jj/releases/download/${JJ_VER}/jj-${JJ_VER}-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]').tar.gz" | tar zxf - jj
...
KCTL_VER="v1.34.3"; curl -LO "https://dl.k8s.io/release/${KCTL_VER}/bin/$(uname -s | tr '[:upper:]' '[:lower:]')/$(uname -m)/kubectl"
...
RUFF_VER="0.14.9"; curl -L "https://github.com/astral-sh/ruff/releases/download/${RUFF_VER}/ruff-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]').tar.gz" | tar zxf - --strip-components 1 ruff-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]')/ruff
...
TART_VER="2.30.1"; curl -L "https://github.com/cirruslabs/tart/releases/download/${TART_VER}/tart.tar.gz" | tar zxf - tart.app
xattr -c tart.app

TFORM_VER="1.15.0"; curl -LOSfs "https://releases.hashicorp.com/terraform/${TFORM_VER}/terraform_${TFORM_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip" && unzip -oqK terraform_${TFORM_VER}_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m).zip terraform
...
TRIVY_VER="0.68.1"; curl -L "https://github.com/aquasecurity/trivy/releases/download/v${TRIVY_VER}/trivy_${TRIVY_VER}_macOS-ARM64.tar.gz" | tar zxf - trivy
...
UV_VER="0.9.18"; curl -L https://github.com/astral-sh/uv/releases/download/${UV_VER}/uv-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]').tar.gz | tar zxf - --strip-components 1 uv-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]')/{uv,uvx}
...
YQ_VER="v4.50.1"; curl -L "https://github.com/mikefarah/yq/releases/download/${YQ_VER}/yq_$(uname -s | tr '[:upper:]' '[:lower:]')_$(uname -m)" -o yq

ZIZMOR_VER="v1.18.0"; curl -L "https://github.com/zizmorcore/zizmor/releases/download/${ZIZMOR_VER}/zizmor-aarch64-apple-$(uname -s | tr '[:upper:]' '[:lower:]').tar.gz" | tar zxf -
```
